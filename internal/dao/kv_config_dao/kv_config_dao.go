	...
	newKvConfigDaoOnce.Do(func() {
		k = &KvConfigDao{
			LocalcacheDao: dao.LocalcacheDao{
				LocalcacheKey: localcache.KvConfig,
			},
		}
	})
	return k
...
func (k *KvConfigDao) GetKvConfigByID(ctx *ctm_context.Context, id uint64) (*KvConfigTab, *err_code.Error) {
	var kv KvConfigTab
	if err := ctx.DB(db_label.ConfigDB).Where("id=?", id).Take(&kv).Error; err != nil {
...
func (k *KvConfigDao) CreateKvConfig(ctx *ctm_context.Context, kv *KvConfigTab) *err_code.Error {
	...
	if err := ctx.DB(db_label.ConfigDB).Create(kv).Error; err != nil {
		return err_code.WriteDBError.WithErr(err)
	}

	k.RefreshLocalcacheVersion(ctx)
	return nil
}
...
func (k *KvConfigDao) UpdateKvConfig(ctx *ctm_context.Context, kv *KvConfigTab) *err_code.Error {
	...
	if err := ctx.DB(db_label.ConfigDB).Save(kv).Error; err != nil {
		return err_code.WriteDBError.WithErr(err)
	}
		...
		for _, v := range data {
			...
			if v.Status == constant.Enable {
				...
				case kv_config.String:
					...
					ok = true
				default:
					ctx.Logger.Error("Unknow value type", zap.Uint8("valueType", v.ValueType))
					continue
				}
				if !ok {
					ctx.Logger.Error("Can't convert", zap.Uint8("valueType", v.ValueType), zap.String("value", v.Value))
					continue
				}
...
func (k *KvConfigDao) getCacheValue(ctx *ctm_context.Context, configKey string) (interface{}, *err_code.Error) {
	m, err := k.Localcache.Get(k.LocalcacheKey)
	if err != nil {
		return nil, err_code.GetLocalcacheError.WithErr(err)
	}
	mp := m.(map[string]interface{})
	if value, ok := mp[configKey]; ok {
		return value, nil
	}
	return nil, err_code.KvConfigKeyNotExistError.Withf("configKey:%s", configKey)
